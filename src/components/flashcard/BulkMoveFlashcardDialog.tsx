...
interface BulkMoveFlashcardDialogProps {
  isOpen: boolean;
  onClose: () => void;
...
/**
 * Dialog for moving multiple flashcards to a different project
 * Fetches and displays a list of available projects (excluding the current one)
 */
export default function BulkMoveFlashcardDialog({
  isOpen,
  onClose,
  onSubmit,
  isSubmitting,
      ...
      <DialogContent className="sm:max-w-[500px]">
        <form onSubmit={handleSubmit}>
          <DialogHeader>
            ...
            <DialogDescription>
              Move {selectedCount} selected flashcard{selectedCount !== 1 ? "s" : ""} to a different project.
            </DialogDescription>
          </DialogHeader>
          ...
          <DialogFooter>
            ...
            <Button type="submit" disabled={isSubmitting || !selectedProjectId || projects.length === 0}>
              {isSubmitting ? `Moving ${selectedCount}...` : `Move ${selectedCount}`}
            </Button>
          </DialogFooter>
        </form>
      </DialogContent>
